...
int mbedtls_aes_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( mode == MBEDTLS_AES_DECRYPT )
        {
            memcpy( buf, aes_test_ctr_ct[u], (size_t)len );
            aes_tests = aes_test_ctr_pt[u];
        }
        else
        {
            memcpy( buf, aes_test_ctr_pt[u], (size_t)len );
            aes_tests = aes_test_ctr_ct[u];
        }

        ret = mbedtls_aes_crypt_ctr( &ctx, (size_t)len, &offset, nonce_counter,
                                     stream_block, buf, buf );
        if( ret != 0 )
            goto exit;

        if( memcmp( buf, aes_tests, (size_t)len ) != 0 )
        {
            ret = 1;
            goto exit;
        }
