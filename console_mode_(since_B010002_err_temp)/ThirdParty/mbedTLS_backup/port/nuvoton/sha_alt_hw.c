...
{	/* Init crypto module */
#ifdef SHA_DISPLAY_PROCESS_DBG
	dbgMsg("%s", "INIT SHA256_224\r\n");
#endif
	crypto_init();
	memset(ctx, 0, sizeof(*ctx));
}
...
{	// Accept only:
	...
	uint32_t DGSTs[16] = { 0 };
#ifdef CRYPTO_DISPLAY_PROCESS_TIME
	uint32_t processTime;
#endif
    
#if defined MBEDTLS_SHA256_ALT && defined MBEDTLS_SHA512_ALT
	if(ctx->is256)
	{	if((sha_opmode == SHA_MODE_SHA512) || (sha_opmode == SHA_MODE_SHA384))
		{	dbgMsg("%s(%d), prev is SHA512, change to SHA256\r\n", __func__, __LINE__);
			mbedtls_sha256_hw_starts(ctx, ctx->is224_384);
		}
	}
	else
	{	if((sha_opmode == SHA_MODE_SHA256) || (sha_opmode == SHA_MODE_SHA224))
		{	dbgMsg("%s(%d), prev is SHA256, change to SHA512\r\n", __func__, __LINE__);
			mbedtls_sha512_hw_starts(ctx, ctx->is224_384);
		}
	}
#endif
		
#ifdef CRYPTO_DISPLAY_PROCESS_TIME
	processTime = GetTimeTicks();
#endif
	while(rmn > 0) 
	...
	{ /* SHA accelerator doesn't export a flag to indicate non-last block process has finished.
		...
		{	switch(sha_opmode) 
			{	case SHA_MODE_SHA512:
							if(DGSTs[12] != CRPT->HMAC_DGST[12] || DGSTs[13] != CRPT->HMAC_DGST[13] || DGSTs[14] != CRPT->HMAC_DGST[14] || DGSTs[15] != CRPT->HMAC_DGST[15]) 
							{	isfinish = 1;
								break;
							}
				case SHA_MODE_SHA384:
							if(DGSTs[8] != CRPT->HMAC_DGST[8] || DGSTs[9] != CRPT->HMAC_DGST[9] || DGSTs[10] != CRPT->HMAC_DGST[10] || DGSTs[11] != CRPT->HMAC_DGST[11]) 
							{	isfinish = 1;
								break;
							}
				case SHA_MODE_SHA256:
				...
				case SHA_MODE_SHA1:
							if(DGSTs[0] != CRPT->HMAC_DGST[0] || DGSTs[1] != CRPT->HMAC_DGST[1] || DGSTs[2] != CRPT->HMAC_DGST[2] || DGSTs[3] != CRPT->HMAC_DGST[3] || DGSTs[4] != CRPT->HMAC_DGST[4]) 
							{	isfinish = 1;
								break;
							}
			}
			if((GetTimeTicks() - currTick) > 2000)
...
#ifdef CRYPTO_DISPLAY_PROCESS_TIME
	dbgMsg("%s(%d), SHA %d time %d\r\n", __func__, __LINE__, ctx->is256? (ctx->is224_384? 224:256):(ctx->is224_384? 384:512),  GetTimeTicks() - processTime);
#endif
}
