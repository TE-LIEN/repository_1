...
#if defined(NU_CRYPTO_RSA_ENABLE)
    hw_encrypt_norm_capable = crypto_rsa_encrypt_norm_capable(ctx);
    if (!ctx->hw_init) {
        ctx->hw_init = (crypto_rsa_init(ctx) == 0) ? 1 : 0;
    }
                 ...
                 unsigned char *output )
{
...
#if defined(NU_CRYPTO_RSA_ENABLE)
    bool hw_go = false;
    bool hw_decrypt_norm_capable = false;
#if !defined(MBEDTLS_RSA_NO_CRT)
...
#if defined(NU_CRYPTO_RSA_ENABLE)
    hw_decrypt_norm_capable = crypto_rsa_decrypt_norm_capable(ctx, f_rng != NULL);
#if !defined(MBEDTLS_RSA_NO_CRT)
    hw_decrypt_crt_capable = crypto_rsa_decrypt_crt_capable(ctx, f_rng != NULL);
#endif
    ...
    hw_go = (hw_decrypt_norm_capable ||
#if !defined(MBEDTLS_RSA_NO_CRT)
        hw_decrypt_crt_capable ||
#endif
        0) && ctx->hw_init;
