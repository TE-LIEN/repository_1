                             ...
                             unsigned char *output, size_t dataSize)
{
    ...
    /* Init crypto module */
#if defined CRYPTO_AES_USE_M460_LIB
		/* Force AES free */
...
#ifdef CRYPTO_DISPLAY_PROCESS_TIME
		dbgMsg("%s(%d) AES %s Time %d\r\n", __func__, __LINE__, ctx->encDec == 1? "ENC":"DEC", GetTimeTicks() - processTime);
#endif
    if( pOut != output ) {
...
#ifndef CRYPTO_AES_USE_M460_LIB
