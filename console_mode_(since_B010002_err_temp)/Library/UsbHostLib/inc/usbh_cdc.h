...
#include "NuMicro.h"
#include "usb.h"
#include "sensCommon.h"	//yushun add
...
#define CDC_DBGMSG      dbgMsg2
#define CDC_ERRMSG		dbgMsg2
#else
#define CDC_DBGMSG(...)
#define CDC_ERRMSG(...)
#endif
...
typedef struct cdc_dev_t
{
	...
	//yushun add for CDC-ECM
	void 				*upperCtx;
	uint8_t				prevDsrSignal;
	uint8_t				currDsrSignal;
	uint8_t				isInitialDone;
	uint8_t				isRxReady;
	void				*uartCfg;
	uint8_t				cdcType;
	uint8_t				iMacAddress;
	uint8_t				macAddr[6];
	uint8_t				prevNetworkIsConnect;
	uint16_t			mtu;
	uint32_t			prevNetworkUlSpeed;
	uint32_t			prevNetworkDlSpeed;
	uint8_t				*cdcEcmRxBuf;
	void 				*netIf;
	SENS_SEM_STRUCT		rxSema;
}   CDC_DEV_T;
