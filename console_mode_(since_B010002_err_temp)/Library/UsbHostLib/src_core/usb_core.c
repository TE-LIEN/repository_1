...
void  usbh_core_init()
{
    ...
    _ohci->HcMiscControl |= USBH_HcMiscControl_OCAL_Msk; /* Over-current active low */	//yushn test
...
void usbh_suspend()
{
    ...
    {
        // USB_DBG_LOCK();
		USB_error("%s", "usbh_suspend - RUN/HCHalted error!\r\n");
		// USB_DBG_UNLOCK();
    }
    delay_us(100);
#endif
 ...
 *  @retval   Otherwise  Failed
 */
int usbh_get_config_descriptor(UDEV_T *udev, uint8_t *desc_buff, int buff_len, int configIndex)
{
    uint32_t  read_len;
    ...
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
						 ((USB_DT_STANDARD | USB_DT_CONFIGURATION) << 8) | (configIndex & 0xFF),
						 0, 9,
    ...
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_IN | REQ_TYPE_STD_DEV | REQ_TYPE_TO_DEV,
                         USB_REQ_GET_DESCRIPTOR,
						 ((USB_DT_STANDARD | USB_DT_CONFIGURATION) << 8) | (configIndex & 0xFF), 0, read_len,
                         desc_buff, &read_len, 200);
    if(ret < 0)
        return ret;
...
int connect_device(UDEV_T *udev, const char *callerFunc, int callerLine)	//yushun modify
{
    DESC_CONF_T  *conf;
    uint32_t     read_len;
    int          ret;
	...
	// USB_DBG_LOCK();
    USB_debug("Connect device call from %s(%d)=>\r\n", callerFunc, callerLine);	//yushun modify
	// USB_DBG_UNLOCK();
	...
	// USB_DBG_LOCK();
    USB_debug("%s", "ready to reset device\r\n");
	// USB_DBG_UNLOCK();

    reset_device(udev);
	
	// USB_DBG_LOCK();
    USB_debug("%s", "ready to set address\r\n");
	// USB_DBG_UNLOCK();
    ...
    {
        // USB_DBG_LOCK();
		USB_debug("%s", "Set address command failed!!\r\n");
		// USB_DBG_UNLOCK();
        return ret;
    }
    ...
    ret = usbh_get_config_descriptor(udev, (uint8_t *)conf, MAX_DESC_BUFF_SIZE, 0);	//yushun modify
    if(ret < 0)
    {
        free_dev_address(udev->dev_num);
        return ret;
...
#endif
	//yushun add start for RTL8153 CDC-ECM at interface 1
	if(udev->descriptor.bNumConfigurations != 1)
	{	conf = (DESC_CONF_T *)usbh_alloc_mem(MAX_DESC_BUFF_SIZE);
		if(conf == NULL)
		{	free_dev_address(udev->dev_num);
		...
		udev->cfdBuff1 = (uint8_t *)conf;
		
		ret = usbh_get_config_descriptor(udev, (uint8_t *)conf, MAX_DESC_BUFF_SIZE, 1);
		if(ret < 0)
		{	free_dev_address(udev->dev_num);
			return ret;
		}
		...
		dump_config_descriptor(conf);
#endif
		if((udev->descriptor.idVendor == 0x0BDA) && (udev->descriptor.idProduct == 0x8153))	//RTL8153 change to config 1, CDC-ECM
		{	usbh_free_mem(udev->cfd_buff, MAX_DESC_BUFF_SIZE);
			udev->cfd_buff = (uint8_t *)conf;
		}
		else
			conf = (DESC_CONF_T *)udev->cfd_buff;
	}
	//yushun add end
	...
	//yushun add start
	if((udev->descriptor.idVendor == 0x0BDA) && (udev->descriptor.idProduct == 0x8153))
	{	usbh_get_string_descriptor(udev, 3, str_buff, MAX_DESC_BUFF_SIZE);
		print_usb_string("MAC: ", str_buff);
	}
	//yushun add end
    ...
    if(conf->bmAttributes & (1 << 5))
    {
        ...
        {
            // USB_DBG_LOCK();
			USB_debug("%s", "Device does not accept remote wakeup enable command.\r\n");
			// USB_DBG_UNLOCK();
        }
    }
