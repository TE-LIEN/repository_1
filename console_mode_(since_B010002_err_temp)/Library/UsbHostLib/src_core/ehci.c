...
{
	// USB_DBG_LOCK();
    USB_debug("%s", "Dump HSUSBH(EHCI) registers:\r\n");
...
void dump_ehci_asynclist_simple(void)
{
    ...
    // USB_DBG_LOCK();
	USB_debug("%s", ">>> EHCI Asynchronous List <<<\r\n");
    USB_debug("%s", "[QH] => ");
    do
    {
...
void dump_ehci_period_frame_list_simple(void)
{
    ...
    // USB_DBG_LOCK();
	USB_debug("%s", ">>> EHCI period frame list simple <<<\r\n");
    USB_debug("%s", "[FList] => ");
    do
    {
        USB_debug("0x%08x ", (int)qh);
