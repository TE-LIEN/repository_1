...
#define HUB_DBGMSG     dbgMsg2
#define HUB_DBGMSG1     dbgMsg
//#define HUB_DBGMSG(...)
...
int hub_probe(IFACE_T *iface)
{
    ...
    {
        // USB_DBG_LOCK();
		USB_error("%s", "Failed to get hub descriptor!\r\n");
		// USB_DBG_UNLOCK();
    }
    else
    {
    ...
    {
        // USB_DBG_LOCK();
		HUB_DBGMSG("Error - failed to submit interrupt read request (%d)\r\n", ret);
		// USB_DBG_UNLOCK();
        free_utr(utr);
        free_hub_device(hub);
...
void hub_disconnect(IFACE_T *iface)
{
    ...
    {
        // USB_DBG_LOCK();
		HUB_DBGMSG("%s", "hub_disconnect - hub not found!\r\n");
		// USB_DBG_UNLOCK();
        return;
    }
...
static int  port_connect_change(HUB_DEV_T *hub, int port, uint16_t wPortStatus)
{
    ...
    if(wPortStatus & PORT_S_CONNECTION)
    {
        ...
        ret = connect_device(udev, __func__, __LINE__);	//yushun modify
        if(ret < 0)
        {
            // USB_DBG_LOCK();
    ...
    for(i = 0; i < MAX_HUB_DEVICE; i++)
    {
        if((g_hub_dev[i].iface != NULL) && (g_hub_dev[i].sc_bitmap))
        {
            ...
            if(ret == 0)
            {
                ...
                {
                    // USB_DBG_LOCK();
					USB_error("Failed to re-submit HUB [%s] interrupt-in request (%d)\r\n", hub->pos_id, ret);
					// USB_DBG_UNLOCK();
                }
            }
        }
