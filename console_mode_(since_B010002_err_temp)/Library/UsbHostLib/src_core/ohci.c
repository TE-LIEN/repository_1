...
static int ohci_reset(void)
{
    ...
    {
        // USB_DBG_LOCK();
		USB_error("%s", "Error! - USB OHCI reset timed out!\r\n");
		// USB_DBG_UNLOCK();
        return -1;
    }
    ...
    {
        // USB_DBG_LOCK();
		USB_error("%s", "Error! - USB HC reset timed out!\r\n");
		// USB_DBG_UNLOCK();
        return -1;
    }
    return 0;
...
static int ohci_rh_polling(void)
{
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        if(_ohci->HcRhPortStatus[i] & USBH_HcRhPortStatus_CCS_Msk)
        {
            ...
            ret = connect_device(udev, __func__, __LINE__);
            if(ret < 0)
            {
                // USB_DBG_LOCK();
...
{
    // USB_DBG_LOCK();
	USB_debug("%s", "Dump OCHI registers:\r\n");
