...
uint8_t  *g_uvc_buff_pool[UVC_MAX_DEVICE];
...
static UVC_DEV_T *alloc_uvc_device(UDEV_T *udev)
{
    ...
    {
		if(g_uvc_buff_pool[i] == NULL)
		{	g_uvc_buff_pool[i] = SENS_MEM_ZALLOC(UVC_UTR_PER_STREAM * UVC_UTR_INBUF_SIZE);
		}
    ...
    {
        // USB_DBG_LOCK();
		UVC_DBGMSG("%s", "Failed to allocate UVC iso-in buffer!\r\n");
		// USB_DBG_UNLOCK();
...
static int  uvc_probe(IFACE_T *iface)
{
    ...
    if(ifd->bInterfaceSubClass == UVC_SC_VIDEO_IF_COLLECT)
    {
        ...
        {
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "Warning UVC IAD - interface count is not 2!\r\n");
			// USB_DBG_UNLOCK();
        }
