...
int uvc_parse_control_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    ...
    if (size < (int)sizeof(DESC_IF_T))
    {
        ...
        // USB_DBG_LOCK();
		UVC_ERRMSG("%s", "UVC_RET_PARSER! - Can't find the Standard Video Control Interface!\r\n");
		// USB_DBG_UNLOCK();
    ...
    // USB_DBG_LOCK();
	UVC_DBGMSG("%s", "VC Interface VC_HEADER\r\n");
    ...
    while (size >= (int)sizeof(DESC_HDR_T))
    {
        ...
        case VC_INPUT_TERMINAL:
            ifd_it = (DESC_VC_IT_T *)ifd;
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VC Interface VC_INPUT_TERMINAL\r\n");
        ...
        case VC_OUTPUT_TERMINAL:
            ...
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VC Interface VC_OUTPUT_TERMINAL\r\n");
            UVC_DBGMSG("    bTerminalID:    0x%x\r\n", ifd_ot->bTerminalID);
        ...
        case VC_SELECTOR_UNIT:
            ...
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VC Interface VC_SELECTOR_UNIT\r\n");
        ...
        case VC_PROCESSING_UNIT:
			...
			// USB_DBG_LOCK();
            UVC_DBGMSG("%s", "VC Interface VC_PROCESSING_UNIT\\rn");
            UVC_DBGMSG("    bUnitID:          0x%x\r\n", ifd_pu->bUnitID);
        ...
        case VC_EXTENSION_UNIT:
            ...
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VC Interface VC_EXTENSION_UNIT\r\n");
            UVC_DBGMSG("    bUnitID:      0x%x\r\n", ifd_eu->bUnitID);
        ...
        case VC_ENCODING_UNIT:
            ...
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VC Interface VC_ENCODING_UNIT\r\n");
            UVC_DBGMSG("    bUnitID:   0x%x\r\n", ifd_ecu->bUnitID);
            UVC_DBGMSG("    bSourceID: 0x%x\r\n", ifd_ecu->bSourceID);
...
int uvc_parse_streaming_interface(UVC_DEV_T *vdev, IFACE_T *iface)
{
    ...
    {
        // USB_DBG_LOCK();
		UVC_ERRMSG("%s", "UVC_RET_PARSER! - Can't find the Standard Video Streaming Interface!\r\n");
		// USB_DBG_UNLOCK();
        return UVC_RET_PARSER;
    }
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        case VS_INPUT_HEADER:
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VS Interface VS_INPUT_HEADER\r\n");
        ...
        case VS_FORMAT_UNCOMPRESSED:
			...
			// USB_DBG_LOCK();
            UVC_DBGMSG("%s", "VS Interface VS_FORMAT_UNCOMPRESSED\r\n");
            ...
            for (i = 0; (i < vsu_format->bNumFrameDescriptors) && (vc->num_of_frames < UVC_MAX_FRAME); i++)
            {
                ...
                {
                    // USB_DBG_LOCK();
					UVC_DBGMSG("%s", "Frame parsing error!!\r\n");
					// USB_DBG_UNLOCK();
                    return UVC_RET_PARSER;
                }

                // USB_DBG_LOCK();
				UVC_DBGMSG("%s", "VS Interface VS_FRAME_UNCOMPRESSED\r\n");
        ...
        case VS_FORMAT_MJPEG:
            ...
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "VS Interface VS_FORMAT_MJPEG\r\n");
            ...
            for (i = 0; (i < mjpg_format->bNumFrameDescriptors) && (vc->num_of_frames < UVC_MAX_FRAME); i++)
            {
                ...
                // USB_DBG_LOCK();
				UVC_DBGMSG("%s", "VS Interface VS_FRAME_MJPEG\r\n");
            ...
            {
                UVC_DBGMSG("%s", "No VS_STILL_IMAGE_FRAME found (optional), skipping.\n");
                ifd = (DESC_IF_T *)bptr;  /* must */
                break;
            }

            UVC_DBGMSG("%s", "VS Interface VS_STILL_IMAGE_FRAME\n");
...
//            vc->num_of_frames++;
    ...
    while (size >= (int)sizeof(DESC_IF_T))
    {
        ...
        if (size < 8)
        {
            /* Parsing error                              */
            // USB_DBG_LOCK();
			UVC_ERRMSG("%s", "UVC_RET_PARSER! - VS descriptor length!\r\n");
			// USB_DBG_UNLOCK();
            return UVC_RET_PARSER;
        }
        ...
        {
            UVC_DBGMSG("%s", "  [!] bNumEndpoints is not 1!\r\n");
        }
		// USB_DBG_UNLOCK();
        ...
        {
            // USB_DBG_LOCK();
			UVC_DBGMSG("%s", "  [!] Endpoint descriptor not found!\r\n");
			// USB_DBG_UNLOCK();
    ...
    // USB_DBG_LOCK();
	UVC_DBGMSG("%s", "\r\n\r\n----------------------------------------------------------\r\n");
    UVC_DBGMSG("%s", "[Video Streaming interface parsing result dump]\r\n");
