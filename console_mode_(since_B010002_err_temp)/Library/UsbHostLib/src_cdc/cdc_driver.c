...
#include "usbh_lib.h"
#include "usbh_cdc.h"
#include "usbHost/usbHostTask.h"	//yushun add
...
static int  cdc_probe(IFACE_T *iface)
{
	...
	//yushun add start
	if((udev->descriptor.idVendor == UBLOX_L210_VID) && (udev->descriptor.idProduct == UBLOX_L210_PID1))
	{	if(iface->if_num >= 6)
			return USBH_ERR_NOT_MATCHED;
	}
	
	//for CDC_ECM
	if((udev->descriptor.idVendor == REALTEK_VID) && (udev->descriptor.idProduct == RTL8153_PID))
	{	if(iface->num_alt > 1)
		{	usbh_set_interface(iface, 1);
		}
	}
	//yushun add end
	...
	cdev->cdcType = ifd->bInterfaceSubClass;	//yushun add
	...
	//yushun add start, for CDC-ECM
	if(cdev->cdcType == CDC_SC_ETHER)
	{	uint8_t *str_buff;
		int len, i=2;
		uint8_t macAddrIdx=0;
		uint8_t order=0;
		char *end;
		uint8_t convertValue;
		str_buff = (uint8_t *)usbh_alloc_mem(MAX_DESC_BUFF_SIZE);
		usbh_get_string_descriptor(udev, cdev->iMacAddress, str_buff, MAX_DESC_BUFF_SIZE);
		
		USB_DBG_LOCK();
		CDC_DBGMSG("%s", "MAC: ");
		len = str_buff[0];
		while(i < len)
		{	convertValue = (uint8_t)strtol((const char *)&str_buff[i], &end, 16);
			CDC_DBGMSG("%c", str_buff[i]);
			if(order == 0)
			{	cdev->macAddr[macAddrIdx] = (convertValue & 0x0F) << 4;
				order = 1;
			}
			else
			{	cdev->macAddr[macAddrIdx] |= (convertValue & 0x0F);
				order = 0;
				macAddrIdx++;
			}
			i += 2;
		...
		CDC_DBGMSG("%s", "\r\n");
		CDC_DBGMSG("MAC: %02X:%02X:%02X:%02X:%02X:%02X\r\n", cdev->macAddr[0], cdev->macAddr[1], cdev->macAddr[2], cdev->macAddr[3], cdev->macAddr[4], cdev->macAddr[5]);
		USB_DBG_UNLOCK();
