...
#if defined OS_MQX
#include "posix.h"	//MQX, like errno.h
#else
#include <errno.h>
#endif
//#include <stdio.h>
...
#include "sensors/modbus/modbus-tcp.h"
#include "sensors/modbus/modbus-tcp-private.h"

#if defined NET_LWIP
#include "lwip/sockets.h"
	#define MODBUS_SOCK_CLOSE         closesocket
#elif defined NET_RTCS
  #define MODBUS_SOCK_CLOSE(sock)   shutdown(sock, 2)
#endif
...
{	/* MSG_NOSIGNAL
	...
	return send(ctx->s, (char *)req, req_length, MSG_NOSIGNAL);
}
...
{	/* Check transaction ID */
	...
	{	if(ctx->debug) 
		{	dbg_msg("Invalid transaction ID received 0x%X (not 0x%X)\n", (rsp[0] << 8) + rsp[1], (req[0] << 8) + req[1]);
		}
		errno = EMBBADDATA;
		return -1;
	}
	...
	{	if(ctx->debug) 
		{	dbg_msg("Invalid protocol ID received 0x%X (not 0x0)\n", (rsp[2] << 8) + rsp[3]);
		}
		errno = EMBBADDATA;
		return -1;
	}
...
{	int rc = connect(sockfd, addr, addrlen);
	...
	{	fd_set wset;
		...
		if(selectTimeout == 0)
			selectTimeout = 500;	//500ms
		rc = select(sockfd + 1, NULL, &wset, NULL, selectTimeout);
#endif
...
{	int rc;
	...
	if(rc == -1) 
	{	MODBUS_SOCK_CLOSE(ctx->s);
		ctx->s = -1;
		return -1;
	}

	if(ctx->debug) 
	{	dbg_msg("Connecting to %s:%d\r\n", ctx_tcp->ip, ctx_tcp->port);
	}
...
{	int rc;
	...
	if(rc != 0) 
	{	if(ctx->debug) 
		{	dbg_msg("Error returned by getaddrinfo: %s\r\n", gai_strerror(rc));
		}
		errno = ECONNREFUSED;
		return -1;
	}
...
{	struct sockaddr_in addr;
	...
	if(ctx->debug) 
	{	dbg_msg("The client connection from %s is accepted\r\n", inet_ntoa(addr.sin_addr));
	}
	return ctx->s;
...
{	struct sockaddr_storage addr;
	...
	if(ctx->debug) 
	{	dbg_msg("%s", "The client connection is accepted.\r\n");
	}
	return ctx->s;}
#endif
...
{	int s_rc;
	...
	//while((s_rc = select(ctx->s+1, rset, NULL, NULL, tv)) == -1) 
	while((s_rc = select(ctx->s+1, rset, NULL, NULL, selectTimeout)) == -1) 
#endif
	{	if(errno == EINTR) 
		{	if(ctx->debug) 
			{	dbg_msg("%s", "A non blocked signal was caught\r\n");
			}
...
{	if(ctx->backend_data) 
	{	modbus_tcp_pi_t *ctx_tcp_pi = ctx->backend_data;
		SENS_MEM_FREE(ctx_tcp_pi->node);
		SENS_MEM_FREE(ctx_tcp_pi->service);
		SENS_MEM_FREE(ctx->backend_data);
	}
	SENS_MEM_FREE(ctx);
}
...
{	modbus_t *ctx;
	...
	ctx->backend_data = (modbus_tcp_t *)SENS_MEM_ZALLOC(sizeof(modbus_tcp_t));
	...
	{	dest_size = sizeof(char) * 16;
		ret_size = strlcpy1(ctx_tcp->ip, ip, dest_size);
		if(ret_size == 0) 
		{	dbg_msg("%s", "The IP string is empty\r\n");
			modbus_free(ctx);
			errno = EINVAL;
			return NULL;
		}
		if(ret_size >= dest_size) 
		{	dbg_msg("%s", "The IP string has been truncated\r\n");
			modbus_free(ctx);
			errno = EINVAL;
			return NULL;
		}
...
{	modbus_t *ctx;
	...
	ctx->backend_data = (modbus_tcp_pi_t *)SENS_MEM_ZALLOC(sizeof(modbus_tcp_pi_t));
	if(ctx->backend_data == NULL) 
	{	modbus_free(ctx);
		errno = ENOMEM;
		return NULL;
