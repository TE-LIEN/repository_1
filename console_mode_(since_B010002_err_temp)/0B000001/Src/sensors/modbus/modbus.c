...
#include "sensors/modbus/modbus-private.h"
...
{	if(ctx->debug) 
	{	debugLock();
		dbg_msg1("ERROR %s", modbus_strerror(errno));
		if(context != NULL) 
		{	dbg_msg1(": %s\n", context);
		} 
		else 
...
{	/* Response timeout is always positive */
	...
	delayTime = ctx->response_timeout.tv_sec * 1000;
	delayTime = delayTime + ctx->response_timeout.tv_usec / 1000;
	SENS_TIME_DELAY(delayTime);
}
...
{	int rc;
	...
	/* In recovery mode, the write command will be issued until to be successful! Disabled by default. */
	do 
	{	
#if defined NET_LWIP
		rc = ctx->backend->sendModbus(ctx, msg, msg_length);
#else
...
{	int rc;
	...
	if(ctx->debug) 
	{	if(msg_type == MSG_INDICATION) 
		{	dbg_msg("%s", "Waiting for an indication...\r\n");
		} 
		else 
		{	dbg_msg("%s", "Waiting for a confirmation...\r\n");
		}
	}

	if(ctx->s < 0) 
	{	if(ctx->debug) 
		{	dbg_msg("%s", "ERROR The connection is not established.\r\n");
		}
		return -1;
	}
	...
	{	rc = ctx->backend->modbusSelect(ctx, &rset, p_tv, length_to_read);
		if(rc == -1) 
		...
		rc = ctx->backend->modbusRecv(ctx, msg + msg_length, length_to_read);
		if(rc == 0) 
		{	errno = ECONNRESET;
			rc = -1;
		}
		...
		if(ctx->debug) 
		{	displayBufInHex(&msg[msg_length], rc, __func__, __LINE__);
		}
		/* Sums bytes received */
		msg_length += rc;
...
{	int rc;
	...
	{	int req_nb_value;
		...
		{	if(ctx->debug) 
			{	dbg_msg("Received function not corresponding to the request (0x%X != 0x%X)\r\n", function, req[offset]);
			}
			if(ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) 
			{	_sleep_response_timeout(ctx);
				modbus_flush(ctx);
		...
		if((req_nb_value == rsp_nb_value) && (resp_addr_ok == 1) && (resp_data_ok == 1)) 
		{	rc = rsp_nb_value;
		} 
		else 
		{	if(ctx->debug) 
			{	dbg_msg("Received data not corresponding to the request (%d != %d)\r\n", rsp_nb_value, req_nb_value);
			}
	...
	{	if(ctx->debug) 
		{	dbg_msg("Message length not corresponding to the computed length (%d != %d)\r\n", rsp_length, rsp_length_computed);
		}
		if(ctx->error_recovery & MODBUS_ERROR_RECOVERY_PROTOCOL) 
		{	_sleep_response_timeout(ctx);
			modbus_flush(ctx);
...
{	int rsp_length;
	...
	{	va_list ap;
		...
		memset(sensSys->dbg.modbusLibMsg, 0, 200);
		vsprintf(sensSys->dbg.modbusLibMsg, template, ap);
		dbg_msg("%s", sensSys->dbg.modbusLibMsg);
		va_end(ap);
	}
	...
	{	case MODBUS_FC_READ_COILS:
		case MODBUS_FC_READ_DISCRETE_INPUTS: 
					{	unsigned int is_input = (function == MODBUS_FC_READ_DISCRETE_INPUTS);
						...
						int nb = (req[offset + 3] << 8) + req[offset + 4];
						/* The mapping can be shifted to reduce memory consumption and it doesn't always start at address zero. */
		...
		case MODBUS_FC_READ_INPUT_REGISTERS: 
					{	unsigned int is_input = (function == MODBUS_FC_READ_INPUT_REGISTERS);
						...
						int nb = (req[offset + 3] << 8) + req[offset + 4];
						/* The mapping can be shifted to reduce memory consumption and it doesn't always start at address zero. */
						if(is_input && mb_mapping->is2SetsInputRegisters)	//old A4 use
						{	if(address >= mb_mapping->start_input_registers_2)
							{	address -= mb_mapping->start_input_registers_2;
							}
						}
		...
		case MODBUS_FC_READ_EXCEPTION_STATUS:
					if(ctx->debug) 
					{	dbg_msg("%s", "FIXME Not implemented\r\n");
					}
					errno = ENOPROTOOPT;
					return -1;
					break;
...
/* Reads IO status */
static int read_io_status(modbus_t *ctx, int function, int addr, int nb, uint8_t *dest, uint8_t send)
{	int rc;
	int req_length;
	...
	req_length = ctx->backend->build_request_basis(ctx, function, addr, nb, ctx->req);
	if(send)
  { rc = send_msg(ctx, ctx->req, req_length);
	}
	else
		rc = 1;
	if(rc > 0) 
	{	int i, temp, bit;
		...
		rc = _modbus_receive_msg(ctx, ctx->rsp, MSG_CONFIRMATION);
		if(rc == -1)
			return -1;

		rc = check_confirmation(ctx, ctx->req, ctx->rsp, rc);
		if(rc == -1)
			return -1;
...
int modbus_read_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest, uint8_t send)
{	int rc;
	...
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Too many bits requested (%d > %d)\r\n", nb, MODBUS_MAX_READ_BITS);
		}
		errno = EMBMDATA;
		return -1;
	}

	rc = read_io_status(ctx, MODBUS_FC_READ_COILS, addr, nb, dest, send);
...
int modbus_read_input_bits(modbus_t *ctx, int addr, int nb, uint8_t *dest, uint8_t send)
{	int rc;
	...
	if(nb > MODBUS_MAX_READ_BITS) 
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Too many discrete inputs requested (%d > %d)\r\n", nb, MODBUS_MAX_READ_BITS);
		}
		errno = EMBMDATA;
		return -1;
	}

	rc = read_io_status(ctx, MODBUS_FC_READ_DISCRETE_INPUTS, addr, nb, dest, send);
...
static int read_registers(modbus_t *ctx, int function, int addr, int nb, uint16_t *dest, uint8_t send)
{	int rc;
	int req_length;
	//uint8_t req[_MIN_REQ_LENGTH];
	//uint8_t rsp[MAX_MESSAGE_LENGTH];
	...
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Too many registers requested (%d > %d)\r\n", nb, MODBUS_MAX_READ_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
	}
	...
	{	int offset;
		...
		rc = _modbus_receive_msg(ctx, ctx->rsp, MSG_CONFIRMATION);
    //dbg_msg("_modbus_receive_msg rc :%d\r\n", rc);
		if(rc < 0)
			return rc;

		rc = check_confirmation(ctx, ctx->req, ctx->rsp, rc);
    //dbg_msg("check_confirmation rc :%d\r\n", rc);
		if(rc == -1)
			return -1;

		offset = ctx->backend->header_length;
		//dbg_msg("offset:%d\r\n", offset);
		//displayBufInHex(rsp, MAX_MESSAGE_LENGTH, __func__, __LINE__);
		for(i = 0; i < rc; i++) 
		{	/* shift reg hi_byte to temp OR with lo_byte */
			dest[i] = (ctx->rsp[offset + 2 + (i << 1)] << 8) | ctx->rsp[offset + 3 + (i << 1)];
		}
	}
	return rc;
}
...
/* Reads the holding registers of remote device and put the data into an
   array */
int modbus_read_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest, uint8_t send)
{	int status;
	...
	status = read_registers(ctx, MODBUS_FC_READ_HOLDING_REGISTERS, addr, nb, dest, send);
	return status;
}

/* Reads the input registers of remote device and put the data into an array */
int modbus_read_input_registers(modbus_t *ctx, int addr, int nb, uint16_t *dest, uint8_t send)
{	int status;
	...
	if(nb > MODBUS_MAX_READ_REGISTERS) 
	{	dbg_msg("ERROR Too many input registers requested (%d > %d)\r\n", nb, MODBUS_MAX_READ_REGISTERS);
		errno = EMBMDATA;
		return -1;
	}
	status = read_registers(ctx, MODBUS_FC_READ_INPUT_REGISTERS, addr, nb, dest, send);
...
static int write_single(modbus_t *ctx, int function, int addr, const uint16_t value, uint8_t send)
{	int rc;
	int req_length;
	//uint8_t req[_MIN_REQ_LENGTH];
	...
	req_length = ctx->backend->build_request_basis(ctx, function, addr, (int) value, ctx->req);
	if(send)
	{	rc = send_msg(ctx, ctx->req, req_length);
  }
	else
...
int modbus_write_bit(modbus_t *ctx, int addr, int status, uint8_t send)
{	if(ctx == NULL) 
	{	errno = EINVAL;
		return -1;
	}
	return write_single(ctx, MODBUS_FC_WRITE_SINGLE_COIL, addr, status ? 0xFF00 : 0, send);
}

/* Writes a value in one register of the remote device */
int modbus_write_register(modbus_t *ctx, int addr, const uint16_t value, uint8_t send)
{	if(ctx == NULL) 
	{	errno = EINVAL;
		return -1;
	}
	return write_single(ctx, MODBUS_FC_WRITE_SINGLE_REGISTER, addr, value, send);
}
...
{	int rc;
	...
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Writing too many bits (%d > %d)\r\n", nb, MODBUS_MAX_WRITE_BITS);
		}
		errno = EMBMDATA;
		return -1;
	}
...
int modbus_write_registers(modbus_t *ctx, int addr, int nb, const uint16_t *src, uint8_t send)
{	int rc;
	int i;
	int req_length;
	int byte_count;
	...
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Trying to write to too many registers (%d > %d)\r\n", nb, MODBUS_MAX_WRITE_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
	}
	...
	{	//uint8_t rsp[MAX_MESSAGE_LENGTH];
		...
		rc = check_confirmation(ctx, req, ctx->rsp, rc);
	}
	return rc;
}
...
{	int rc;
	...
	{	if(ctx->debug) 
		{	dbg_msg("ERROR Too many registers to write (%d > %d)\r\n", write_nb, MODBUS_MAX_WR_WRITE_REGISTERS);
		}
		errno = EMBMDATA;
		return -1;
	}
...
{	int rc;
	...
	{	int i;
		...
		/* Byte count, slave id, run indicator status and additional data. Truncate copy to max_dest. */
...
{	if(ctx == NULL) 
	...
	return ctx->backend->modbusConnect(ctx);
}

void modbus_close(modbus_t *ctx)
{	if (ctx == NULL)
		return;
	ctx->backend->modbusClose(ctx);
}
