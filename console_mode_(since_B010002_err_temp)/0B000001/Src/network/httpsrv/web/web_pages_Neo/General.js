function makeRequest(url, data)
{
  ...
  //url = url + "?sid=" + Math.floor((Math.random()*10000)+1)+ "&T=" + window.sessionStorage.getItem("T")+"&D=" + data;
  url = url + "?sid=" + window.sessionStorage.getItem("S")+ "&T=" + window.sessionStorage.getItem("T")+"&D=" + data;
	is_lan_alive = 0;
  ...
  if (window.XMLHttpRequest)
  {
	...
	rspTimer = setTimeout("rspTimeout()", 4000);
	return 0;
}
...
{	let authErr = rsp.indexOf("Unauthorized");
	...
	var parsed = rsp.substring(tPosE+1, rsp.length);
	
	if((authErr != -1) && (authErr < 50))
	{	if(unauthorized)
			return
		unauthorized = true
		DtTimerStop = true;
		clearTimeout(DTTimer);
		clearTimeout(rspTimer);
	...
	{	data_received = 0;
		...
		{	var img;
			...
			//imgBin = 'data:image/jpeg;base64,' + btoa(unescape(encodeURIComponent(imgBin)));
			imgBin
			img.src = imgBin;
		}
		else
...
{	var yesObj = document.getElementById(yesObjId);
	var noObj = document.getElementById(noObjId);
	
	if(yesObj.checked)
		return yesObj.value;
	else if(noObj.checked)
		return noObj.value;
	else
		return defaultVal;
}
...
{
	var sObj = document.getElementById(selObj);
	var iObj = document.getElementById(inpObj);
	if (sObj.selectedIndex == 0) iObj.value = "";
}
function Trim(str)
{
