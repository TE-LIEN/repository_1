...
httpsrv_tls_ctx_t httpsrv_tls_init(const HTTPSRV_TLS_PARAM_STRUCT *params)
{
    ...
    if(params)
    {
        ...
        if (ctx && params->certificate_buffer && params->certificate_buffer_size && params->private_key_buffer && params->private_key_buffer_size )
        {
            /* Add cert to ctx */
            if (wolfSSL_CTX_use_certificate_buffer(ctx, params->certificate_buffer, params->certificate_buffer_size, SSL_FILETYPE_PEM) != SSL_SUCCESS)
            {
                goto ERROR;
            }
            
            /* Load server key into ctx */
            if (wolfSSL_CTX_use_PrivateKey_buffer(ctx, params->private_key_buffer, params->private_key_buffer_size, SSL_FILETYPE_PEM) != SSL_SUCCESS)
            {
                goto ERROR;
            }
        }
        ...
        if(ctx)
        {
            ...
            if ( params->certificate_buffer && params->certificate_buffer_size && params->private_key_buffer && params->private_key_buffer_size )
            {
...
/* Seed the RNG  */
				...
				/* Setup stuff */
                if (mbedtls_ssl_config_defaults(&ctx->conf, MBEDTLS_SSL_IS_SERVER, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT) != 0)
                {
                    goto ERROR;
                }
#endif
				if (mbedtls_x509_crt_parse(&ctx->srvcert, params->certificate_buffer, params->certificate_buffer_size) != 0)
                {
                    goto ERROR;
                }
#if defined MBEDTLS_V3_1
				if (mbedtls_pk_parse_key(&ctx->pkey, params->private_key_buffer, params->private_key_buffer_size, NULL, 0, mbedtls_ctr_drbg_random, &ctx->ctr_drbg) != 0)
                {
                    goto ERROR;
                }
#else
                if (mbedtls_pk_parse_key(&ctx->pkey, params->private_key_buffer, params->private_key_buffer_size, NULL, 0) != 0)
                {
                    goto ERROR;
                }
#endif
