...
{	MQTT_CODE_SUCCESS = 0,
	...
	MQTT_CODE_WAIT = -201,
	MQTT_CODE_WAIT_SOCK_READ_REMAIN_LEN = -202,
};
...
/* Standard wrappers */
    ...
    #include <string.h>
        ...
        #define WOLFMQTT_MALLOC(s)  SENS_MEM_ALLOC((s))
    #endif
    #ifndef WOLFMQTT_FREE
        //#define WOLFMQTT_FREE(p)    {void* xp = (p); if((xp)) free((xp));}
        #define WOLFMQTT_FREE(p)    {void* xp = (p); if((xp)) SENS_MEM_FREE((xp));}
    #endif
#endif
...
/* printf */
		...
		#define PRINTF(_f_, ...)  do{}while(0)	//printf( (_f_ LINE_END), ##__VA_ARGS__)
	#endif
#endif
