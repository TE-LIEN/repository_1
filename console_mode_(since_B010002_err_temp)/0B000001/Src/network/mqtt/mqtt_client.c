...
#define READ_WRITE_SEMA		1

extern void showtime();
/* Private functions */

/* forward declarations */
static int MqttClient_Publish_ReadPayload(MqttClient* client, MqttPublish* publish, int timeout_ms);

#ifdef WOLFMQTT_MULTITHREAD
/* These RespList functions assume caller has locked client->lockClient mutex */
static int MqttClient_RespList_Add(MqttClient *client, MqttPacketType packet_type, word16 packet_id, MqttPendResp *newResp, void *packet_obj)
{	MqttPendResp *tmpResp;
	...
	PRINTF("PendResp Add: %p, Type %s (%d), ID %d", newResp, MqttPacket_TypeDesc(packet_type), packet_type, packet_id);
	#endif

	/* verify newResp is not already in the list */
	for(tmpResp = client->firstPendResp; tmpResp != NULL; tmpResp = tmpResp->next)
	{	if(tmpResp == newResp) 
		{
	#ifdef WOLFMQTT_DEBUG_CLIENT
...
static int MqttClient_RespList_Find(MqttClient *client, MqttPacketType packet_type, word16 packet_id, MqttPendResp **retResp)
{	int rc = 0;
	MqttPendResp *tmpResp;
...
#ifdef WOLFMQTT_DEBUG_CLIENT
	PRINTF("PendResp Find: Type %s (%d), ID %d", MqttPacket_TypeDesc(packet_type), packet_type, packet_id);
#endif
	...
	for(tmpResp = client->firstPendResp; tmpResp != NULL; tmpResp = tmpResp->next)
	{	if(packet_type == tmpResp->packet_type && (packet_id == tmpResp->packet_id))
		{
#ifdef WOLFMQTT_DEBUG_CLIENT
			PRINTF("PendResp Found: %p, Type %s (%d), ID %d", tmpResp, MqttPacket_TypeDesc(tmpResp->packet_type), tmpResp->packet_type, tmpResp->packet_id);
#endif
 ...
 */
static int MqttClient_DecodePacket(MqttClient* client, byte* rx_buf, word32 rx_len, void *packet_obj, MqttPacketType* ppacket_type, MqttQoS* ppacket_qos, word16* ppacket_id)
	...
	{	switch(packet_type) 
		{	case MQTT_PACKET_TYPE_CONNECT_ACK:
						{	MqttConnectAck connect_ack, *p_connect_ack = &connect_ack;
							...
							XMEMCPY(&obj->connect_ack, p_connect_ack, sizeof(MqttConnectAck));
							//dbg_msg("MQTT p_connect_ack->return_code:%X, flags:%X\r\n", p_connect_ack->return_code, p_connect_ack->flags);
#ifdef WOLFMQTT_V5
							if(packet_obj && rc >= 0) 
			...
			case MQTT_PACKET_TYPE_PUBLISH:
            	...
            	{	XMEMSET(p_publish, 0, sizeof(MqttPublish));
							...
							//printf("%s is publish type packet, rc:%d\n", __func__, rc);
							if(rc >= 0) 
			...
			case MQTT_PACKET_TYPE_PUBLISH_COMP:
            	...
            	rc = MqttDecode_PublishResp(rx_buf, rx_len, packet_type, p_publish_resp);
            	if(rc >= 0) 
...
#ifdef WOLFMQTT_DEBUG_CLIENT
	PRINTF("MqttClient_DecodePacket: Rc %d, Len %d, Type %s (%d), ID %d, QoS %d", rc, rx_len, MqttPacket_TypeDesc(packet_type), packet_type, packet_id, packet_qos);
#endif

	return rc;
}

static int MqttClient_HandlePacket(MqttClient* client, MqttPacketType packet_type, void *packet_obj, int timeout_ms)
{	int rc = MQTT_CODE_SUCCESS;
	...
	switch(packet_type)
	{	case MQTT_PACKET_TYPE_CONNECT_ACK:
					{	rc = MqttClient_DecodePacket(client, client->rx_buf, client->packet.buf_len, packet_obj, &packet_type, &packet_qos, &packet_id);
						break;
					}
		case MQTT_PACKET_TYPE_PUBLISH:
					{	MqttPublish *publish = (MqttPublish*)packet_obj;
						...
						resp_type = (packet_qos == MQTT_QOS_1) ? MQTT_PACKET_TYPE_PUBLISH_ACK : MQTT_PACKET_TYPE_PUBLISH_REC;
						...
						rc = MqttEncode_PublishResp(client->tx_buf, client->tx_buf_len, resp_type, &publish->resp);
#ifdef WOLFMQTT_DEBUG_CLIENT
						PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(resp_type), resp_type, packet_id, packet_qos);
#endif
						if(rc <= 0) 
						{
#ifdef WOLFMQTT_MULTITHREAD
		...
		case MQTT_PACKET_TYPE_PUBLISH_COMP:
					{	MqttPublishResp publish_resp;
						...
						rc = MqttClient_DecodePacket(client, client->rx_buf, client->packet.buf_len, &publish_resp, &packet_type, &packet_qos, &packet_id);
						if(rc <= 0) 
						{	return rc;
						}

						/* If publish Received or Release QoS then proceed */
						if(packet_type != MQTT_PACKET_TYPE_PUBLISH_REC && packet_type != MQTT_PACKET_TYPE_PUBLISH_REL) 
						{	break;
						}
						...
						rc = MqttEncode_PublishResp(client->tx_buf, client->tx_buf_len, packet_type, &publish_resp);
#ifdef WOLFMQTT_DEBUG_CLIENT
						PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(packet_type), packet_type, packet_id, packet_qos);
#endif
						if(rc <= 0) 
						{
#ifdef WOLFMQTT_MULTITHREAD
...
#ifdef WOLFMQTT_DEBUG_CLIENT
	if(rc < 0) 
	{	PRINTF("MqttClient_HandlePacket: Rc %d, Type %s (%d), QoS %d, ID %d", rc, MqttPacket_TypeDesc(packet_type), packet_type, packet_qos, packet_id);
	}
#endif
...
static int MqttClient_WaitType(MqttClient *client, void *packet_obj, byte wait_type, word16 wait_packet_id, int timeout_ms)
{	int rc;
	word16 packet_id;
	MqttPacketType packet_type;
#ifdef WOLFMQTT_MULTITHREAD
...
wait_again:
...
#ifdef WOLFMQTT_DEBUG_CLIENT
	PRINTF("MqttClient_WaitType: Type %s (%d), ID %d", MqttPacket_TypeDesc(wait_type), wait_type, wait_packet_id);
#endif
	//printf("%s, %d\n", __func__, *mms_stat);
	switch(*mms_stat)
	{	case MQTT_MSG_BEGIN:
					{	/* reset the packet state */
						...
						//client->packet.stat = MQTT_PK_BEGIN_WAIT;
						client->read.pos = 0;
		...
		case MQTT_MSG_WAIT:
					{
						...
						if(rc == 0) 
						{	if(MqttClient_RespList_Find(client, wait_type, wait_packet_id, &pendResp)) 
						...
						*mms_stat = MQTT_MSG_WAIT;
						...
						/* handle failure */
						//printf("%s call mqttPacketRead rc:%d\n", __func__, rc);
						if(rc <= 0)
						{	
#if 0
							if((rc == MQTT_CODE_WAIT_SOCK_READ_REMAIN_LEN) || (rc == MQTT_CODE_WAIT) || (rc == MQTT_CODE_CONTINUE))
								return rc;
							else
#endif
								break;
						...
						//else
						//{	printf("%s get MqttPacket_Read %d\n", __func__, rc);
						//}
						/* capture length read */
						client->packet.buf_len = rc;
						
						/* Decode Packet - get type and id */
						rc = MqttClient_DecodePacket(client, client->rx_buf, client->packet.buf_len, NULL, &packet_type, NULL, &packet_id);
						if(rc < 0) 
						{	break;
						}
		...
		case MQTT_MSG_READ_PAYLOAD:
					{	MqttPacketType use_packet_type;
						...
						if((wait_type == MQTT_PACKET_TYPE_ANY || wait_type == packet_type) && (wait_packet_id == 0 || wait_packet_id == packet_id))
						...
						rc = wc_LockMutex(&client->lockClient);
						if(rc == 0) 
						{	if(MqttClient_RespList_Find(client, packet_type, packet_id, &pendResp)) 
						...
						rc = MqttClient_HandlePacket(client, use_packet_type, use_packet_obj, timeout_ms);

#ifdef WOLFMQTT_NONBLOCK
						if(rc == MQTT_CODE_CONTINUE) 
						{	/* we have received some data, so keep the recv mutex lock active and return */
							return rc;
						}
#endif
	...
	*mms_stat = MQTT_MSG_BEGIN;
	...
	{
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_WaitType: Failure: %s (%d)", MqttClient_ReturnCodeToString(rc), rc);
#endif
		return rc;
	}
...
/* Public Functions */
int MqttClient_Init(MqttClient *client, MqttNet* net, MqttMsgCb msg_cb, MqttMsgLenErrCb msg_len_err_cb, byte* tx_buf, int tx_buf_len, byte* rx_buf, int rx_buf_len, int cmd_timeout_ms)
{	int rc = MQTT_CODE_SUCCESS;

	/* Check arguments */
	if(client == NULL || tx_buf == NULL || tx_buf_len <= 0 || rx_buf == NULL || rx_buf_len <= 0) 
	{	return MQTT_CODE_ERROR_BAD_ARG;
	}
	...
	client->msg_len_err_cb = msg_len_err_cb;
...
int MqttClient_SetDisconnectCallback(MqttClient *client, MqttDisconnectCb discCb, void* ctx)
{	if(client == NULL)
		return MQTT_CODE_ERROR_BAD_ARG;
...
#ifdef WOLFMQTT_PROPERTY_CB
int MqttClient_SetPropertyCallback(MqttClient *client, MqttPropertyCb propCb, void* ctx)
{	if(client == NULL)
		return MQTT_CODE_ERROR_BAD_ARG;
...
int MqttClient_Connect(MqttClient *client, MqttConnect *mc_connect, int wait)
{	int rc, len = 0;
	...
	if(mc_connect->stat == MQTT_MSG_BEGIN) 
	{
...
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_CONNECT), MQTT_PACKET_TYPE_CONNECT, 0, 0);
#endif
		if(rc <= 0) 
		{
#ifdef WOLFMQTT_MULTITHREAD
		...
		{	/* inform other threads of expected response */
			rc = MqttClient_RespList_Add(client, MQTT_PACKET_TYPE_CONNECT_ACK, 0, &mc_connect->pendResp, &mc_connect->ack);
			wc_UnLockMutex(&client->lockClient);
		}
		if(rc != 0) 
		{	wc_UnLockMutex(&client->lockSend);
	...
	{	MqttAuth auth, *p_auth = &auth;
		...
		for(conn_prop = mc_connect->props; (conn_prop != NULL) && (conn_prop->type != MQTT_PROP_AUTH_METHOD); conn_prop = conn_prop->next) 
		{
		}
		if(conn_prop == NULL) 
		{
...
connectWait:
	...
	//printf("%s, wait\n", __func__);
	rc = MqttClient_WaitType(client, &mc_connect->ack, MQTT_PACKET_TYPE_CONNECT_ACK, 0, client->cmd_timeout_ms);
	//printf("%s, wait done, rc:%d\n", __func__, rc);
#ifdef WOLFMQTT_NONBLOCK
	if(rc == MQTT_CODE_CONTINUE)
		return rc;
#endif
...
{	int rc = MQTT_CODE_SUCCESS;
	...
	{	/* Determine if message is done */
		msg_done = ((publish->buffer_pos + publish->buffer_len) >= publish->total_len) ? 1 : 0;

		if(publish->buffer_new) 
		{	/* Issue callback for new message (first time only) */
			if(client->msg_cb) 
			{	/* if using the temp publish message buffer, then populate message context with client context */
				if(publish->ctx == NULL && &client->msg.publish == publish) 
				{	publish->ctx = client->ctx;
				}
				rc = client->msg_cb(client, publish, publish->buffer_new, msg_done);
...
static int MqttClient_Publish_WritePayload(MqttClient *client, MqttPublish *publish, MqttPublishCb pubCb)
{	int rc = MQTT_CODE_SUCCESS;
	...
	{	word32 tmp_len = publish->buffer_len;
		...
		{	/* Use the callback to get payload */
			...
			{	if(client->write.len > client->tx_buf_len) 
				...
				XMEMCPY(client->tx_buf, &publish->buffer[publish->intBuf_pos], client->write.len);
	...
	{	if(publish->buffer_pos > 0) 
		{	XMEMCPY(client->tx_buf, publish->buffer, client->write.len);
		}

		/* Send packet and payload */
		do 
		{	//printf("%s write buf\n", __func__);
			//for(int idx=0;idx<client->write.len;idx++)
			//{	printf("%02X ", client->tx_buf[idx]);
			//}
			//printf("\n");
			rc = MqttPacket_Write(client, client->tx_buf, client->write.len);
			if(rc < 0) 
...
int MqttClient_Publish_ex(MqttClient *client, MqttPublish *publish, MqttPublishCb pubCb)
{	int rc = MQTT_CODE_SUCCESS;
	MqttPacketType resp_type;
	...
	if((publish->qos > client->max_qos) || ((publish->retain == 1) && (client->retain_avail == 0)))
	...
	{	case MQTT_MSG_BEGIN:
					{
						...
						rc = MqttEncode_Publish(client->tx_buf, client->tx_buf_len, publish, pubCb ? 1 : 0);
						//printf("MqttEncode_Publish rc: %d\n", rc);
#ifdef WOLFMQTT_DEBUG_CLIENT
						PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_PUBLISH), MQTT_PACKET_TYPE_PUBLISH, publish->packet_id, publish->qos);
#endif
						if(rc <= 0) 
						{	dbg_msg("1. publish error rc:%d, stat:%d\r\n", rc, publish->stat);
#ifdef WOLFMQTT_MULTITHREAD
							wc_UnLockMutex(&client->lockSend);
#endif
							return rc;
						...
						{	resp_type = (publish->qos == MQTT_QOS_1) ? MQTT_PACKET_TYPE_PUBLISH_ACK : MQTT_PACKET_TYPE_PUBLISH_COMP;

							rc = wc_LockMutex(&client->lockClient);
							if(rc == 0) 
							{	/* inform other threads of expected response */
								rc = MqttClient_RespList_Add(client, resp_type, publish->packet_id, &publish->pendResp, &publish->resp);
								wc_UnLockMutex(&client->lockClient);
							}
							if(rc != 0) 
							{	wc_UnLockMutex(&client->lockSend);
...
#endif
						if(rc < 0) 
						{	dbg_msg("2. publish error rc:%d, stat:%d\r\n", rc, publish->stat);
#ifdef WOLFMQTT_MULTITHREAD
							wc_UnLockMutex(&client->lockSend);
		...
		case MQTT_MSG_WRITE_PAYLOAD:
					{	rc = MqttClient_Publish_WritePayload(client, publish, pubCb);
#ifdef WOLFMQTT_NONBLOCK
						if(rc == MQTT_CODE_CONTINUE)
						{	dbg_msg("3. publish error rc:%d, stat:%d\r\n", rc, publish->stat);
							return rc;
						}
#endif
#ifdef WOLFMQTT_MULTITHREAD
						...
						if(rc < 0) 
						{	dbg_msg("4. publish error rc:%d, stat:%d\r\n", rc, publish->stat);
#ifdef WOLFMQTT_MULTITHREAD
							if (wc_LockMutex(&client->lockClient) == 0) 
							{	MqttClient_RespList_Remove(client, &publish->pendResp);
								wc_UnLockMutex(&client->lockClient);
		...
		case MQTT_MSG_WAIT:
					{	/* Handle QoS */
						...
						{	/* Determine packet type to wait for */
							...
							rc = MqttClient_WaitType(client, &publish->resp, resp_type, publish->packet_id, client->cmd_timeout_ms);
		...
		case MQTT_MSG_READ_PAYLOAD:
#ifdef WOLFMQTT_DEBUG_CLIENT
					PRINTF("MqttClient_Publish: Invalid state %d!", publish->stat);
#endif
					rc = MQTT_CODE_ERROR_STAT;
					break;
	} /* switch (publish->stat) */
...
#endif
	{	if(rc <= 0)
		{	dbg_msg("publish error rc:%d, set to begin, prev is %d\r\n", rc, publish->stat);
		}
		publish->stat = MQTT_MSG_BEGIN;
	}
	if(rc > 0) 
	{	dbg_msg("publish success rc:%d, stat:%d\r\n", rc, publish->stat);
		rc = MQTT_CODE_SUCCESS;
	}
	else
	{	dbg_msg("6. publish error rc:%d, stat:%d\r\n", rc, publish->stat);
	}
...
{	int rc, len, i;
	...
	if(subscribe->stat == MQTT_MSG_BEGIN) 
	{
...
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_SUBSCRIBE), MQTT_PACKET_TYPE_SUBSCRIBE, subscribe->packet_id, 0);
#endif
		if(rc <= 0) 
		{
#ifdef WOLFMQTT_MULTITHREAD
		...
		if(rc == 0) 
		{	/* inform other threads of expected response */
			rc = MqttClient_RespList_Add(client, MQTT_PACKET_TYPE_SUBSCRIBE_ACK, subscribe->packet_id, &subscribe->pendResp, &subscribe->ack);
			wc_UnLockMutex(&client->lockClient);
		}
		if(rc != 0) 
		{	wc_UnLockMutex(&client->lockSend);
		...
		//printf("%s, subscribe->ack:%d\n", __func__, subscribe->ack);
#if !READ_WRITE_SEMA
		return 0;
#endif
	}
	...
	rc = MqttClient_WaitType(client, &subscribe->ack, MQTT_PACKET_TYPE_SUBSCRIBE_ACK, subscribe->packet_id, client->cmd_timeout_ms);
	//printf("%s, rc:%d\n", __func__, rc);
	if(rc == MQTT_CODE_WAIT)
		return rc;
#ifdef WOLFMQTT_NONBLOCK
...
{	int rc, len;
	...
	if(unsubscribe->stat == MQTT_MSG_BEGIN) 
	{
...
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_UNSUBSCRIBE), MQTT_PACKET_TYPE_UNSUBSCRIBE, unsubscribe->packet_id, 0);
#endif
		if(rc <= 0) 
		{
#ifdef WOLFMQTT_MULTITHREAD
		...
		if(rc == 0) 
		{	/* inform other threads of expected response */
			rc = MqttClient_RespList_Add(client, MQTT_PACKET_TYPE_UNSUBSCRIBE_ACK, unsubscribe->packet_id, &unsubscribe->pendResp, &unsubscribe->ack);
			wc_UnLockMutex(&client->lockClient);
		}
		if(rc != 0) 
	...
	rc = MqttClient_WaitType(client, &unsubscribe->ack, MQTT_PACKET_TYPE_UNSUBSCRIBE_ACK, unsubscribe->packet_id, client->cmd_timeout_ms);
	if(rc == MQTT_CODE_WAIT)
		return rc;
#ifdef WOLFMQTT_NONBLOCK
	if(rc == MQTT_CODE_CONTINUE)
...
{	int rc, len;
	...
	if(ping->stat == MQTT_MSG_BEGIN) 
	{
...
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_PING_REQ), MQTT_PACKET_TYPE_PING_REQ, 0, 0);
#endif
		if(rc <= 0) 
		{
#ifdef WOLFMQTT_MULTITHREAD
		...
		if(rc == 0) 
		{	/* inform other threads of expected response */
			rc = MqttClient_RespList_Add(client, MQTT_PACKET_TYPE_PING_RESP, 0, &ping->pendResp, ping);
			wc_UnLockMutex(&client->lockClient);
		}
		if(rc != 0) 
		{	wc_UnLockMutex(&client->lockSend);
	...
	rc = MqttClient_WaitType(client, ping, MQTT_PACKET_TYPE_PING_RESP, 0, client->cmd_timeout_ms);
	if(rc == MQTT_CODE_WAIT)
		return rc;
#ifdef WOLFMQTT_NONBLOCK
	if(rc == MQTT_CODE_CONTINUE)
...
#ifdef WOLFMQTT_DEBUG_CLIENT
	PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_DISCONNECT), MQTT_PACKET_TYPE_DISCONNECT, 0, 0);
#endif
	if(rc <= 0) 
	{
#ifdef WOLFMQTT_MULTITHREAD
...
{	int rc, len;
	...
	if(auth->stat == MQTT_MSG_BEGIN) 
	{
...
#ifdef WOLFMQTT_DEBUG_CLIENT
		PRINTF("MqttClient_EncodePacket: Len %d, Type %s (%d), ID %d, QoS %d", rc, MqttPacket_TypeDesc(MQTT_PACKET_TYPE_AUTH), MQTT_PACKET_TYPE_AUTH, 0, 0);
#endif
		if(rc <= 0) 
		{
#ifdef WOLFMQTT_MULTITHREAD
		...
		if(rc == 0) 
		{	/* inform other threads of expected response */
			rc = MqttClient_RespList_Add(client, MQTT_PACKET_TYPE_AUTH, 0, &auth->pendResp, auth);
			wc_UnLockMutex(&client->lockClient);
		}
		if(rc != 0) 
		{	wc_UnLockMutex(&client->lockSend);
	...
	rc = MqttClient_WaitType(client, auth, MQTT_PACKET_TYPE_AUTH, 0, client->cmd_timeout_ms);
#ifdef WOLFMQTT_NONBLOCK
	if(rc == MQTT_CODE_CONTINUE)
		return rc;
#endif
...
{	return MqttClient_WaitType(client, msg, MQTT_PACKET_TYPE_ANY, 0, timeout_ms);
}
