...
static const struct MqttPropMatrix gPropMatrix[] = 
    ...
    { MQTT_PROP_PLAYLOAD_FORMAT_IND,        MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_MSG_EXPIRY_INTERVAL,        MQTT_DATA_TYPE_INT,			(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_CONTENT_TYPE,               MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_RESP_TOPIC,                 MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_CORRELATION_DATA,           MQTT_DATA_TYPE_BINARY,	(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE, 		0 },
    { MQTT_PROP_SUBSCRIPTION_ID,            MQTT_DATA_TYPE_VAR_INT,	(1 << MQTT_PACKET_TYPE_PUBLISH) | (1 << MQTT_PACKET_TYPE_SUBSCRIBE) },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_SESSION_EXPIRY_INTERVAL,    MQTT_DATA_TYPE_INT,			(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) | (1 << MQTT_PACKET_TYPE_DISCONNECT) },
    { MQTT_PROP_ASSIGNED_CLIENT_ID,         MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_SERVER_KEEP_ALIVE,          MQTT_DATA_TYPE_SHORT,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE, 		0 },
    { MQTT_PROP_AUTH_METHOD,                MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) | (1 << MQTT_PACKET_TYPE_AUTH) },
    { MQTT_PROP_AUTH_DATA,                  MQTT_DATA_TYPE_BINARY,	(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) | (1 << MQTT_PACKET_TYPE_AUTH) },
    { MQTT_PROP_REQ_PROB_INFO,              MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT) },
    { MQTT_PROP_WILL_DELAY_INTERVAL,        MQTT_DATA_TYPE_INT,			(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_REQ_RESP_INFO,              MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT) },
    { MQTT_PROP_RESP_INFO,                  MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE, 		0 },
    { MQTT_PROP_SERVER_REF,                 MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_CONNECT_ACK) | (1 << MQTT_PACKET_TYPE_DISCONNECT) },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_NONE,												MQTT_DATA_TYPE_NONE,		0 },
    { MQTT_PROP_REASON_STR,                 MQTT_DATA_TYPE_STRING,	(1 << MQTT_PACKET_TYPE_CONNECT_ACK) | (1 << MQTT_PACKET_TYPE_PUBLISH_ACK) |
    																																(1 << MQTT_PACKET_TYPE_PUBLISH_REC) | (1 << MQTT_PACKET_TYPE_PUBLISH_REL) |
    																																(1 << MQTT_PACKET_TYPE_PUBLISH_COMP) | (1 << MQTT_PACKET_TYPE_SUBSCRIBE_ACK) |
    																																(1 << MQTT_PACKET_TYPE_UNSUBSCRIBE_ACK) | (1 << MQTT_PACKET_TYPE_DISCONNECT) |
    																																(1 << MQTT_PACKET_TYPE_AUTH) },
    { MQTT_PROP_NONE, 											MQTT_DATA_TYPE_NONE, 		0 },
    { MQTT_PROP_RECEIVE_MAX,                MQTT_DATA_TYPE_SHORT,		(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_TOPIC_ALIAS_MAX,            MQTT_DATA_TYPE_SHORT,		(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_TOPIC_ALIAS,                MQTT_DATA_TYPE_SHORT,		(1 << MQTT_PACKET_TYPE_PUBLISH) },
    { MQTT_PROP_MAX_QOS,                    MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_RETAIN_AVAIL,               MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_USER_PROP,                  MQTT_DATA_TYPE_STRING_PAIR,	0xFFFF /* ALL */ },
    { MQTT_PROP_MAX_PACKET_SZ,              MQTT_DATA_TYPE_INT,			(1 << MQTT_PACKET_TYPE_CONNECT) | (1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_WILDCARD_SUB_AVAIL,         MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_SUBSCRIPTION_ID_AVAIL,      MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
    { MQTT_PROP_SHARED_SUBSCRIPTION_AVAIL,	MQTT_DATA_TYPE_BYTE,		(1 << MQTT_PACKET_TYPE_CONNECT_ACK) },
...
static int MqttEncode_FixedHeader(byte *tx_buf, int tx_buf_len, int remain_len, byte type, byte retain, byte qos, byte duplicate)
{	int header_len;
...
static int MqttDecode_FixedHeader(byte *rx_buf, int rx_buf_len, int *remain_len, byte type, MqttQoS *p_qos, byte *p_retain, byte *p_duplicate)
{	int header_len;
	...
	*value = 0;
	...
	{	encodedByte = *(buf++);
		*value += (encodedByte & ~MQTT_PACKET_LEN_ENCODE_MASK) * multiplier;
		if(multiplier > (MQTT_PACKET_LEN_ENCODE_MASK * MQTT_PACKET_LEN_ENCODE_MASK * MQTT_PACKET_LEN_ENCODE_MASK))
...
{	int header_len, remain_len;
	...
	remain_len += props_len = MqttEncode_Props(MQTT_PACKET_TYPE_CONNECT, mc_connect->props, NULL);
	...
	{	/* Verify all required fields are present */
		if(mc_connect->lwt_msg == NULL || mc_connect->lwt_msg->topic_name == NULL || mc_connect->lwt_msg->buffer == NULL || mc_connect->lwt_msg->total_len <= 0)
		{	return MQTT_CODE_ERROR_BAD_ARG;
		}
	...
	/* Encode properties */
	tx_payload += MqttEncode_Props(MQTT_PACKET_TYPE_CONNECT, mc_connect->props, tx_payload);
#endif

	/* Encode payload */
	tx_payload += MqttEncode_String(tx_payload, mc_connect->client_id);
	if(mc_connect->enable_lwt) 
	{	tx_payload += MqttEncode_String(tx_payload, mc_connect->lwt_msg->topic_name);
		tx_payload += MqttEncode_Data(tx_payload, mc_connect->lwt_msg->buffer, (word16)mc_connect->lwt_msg->total_len);
	}
	if(mc_connect->username) 
...
int MqttDecode_ConnectAck(byte *rx_buf, int rx_buf_len, MqttConnectAck *connect_ack)
{	int header_len, remain_len;
	byte *rx_payload;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_CONNECT_ACK, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	...
	{	connect_ack->flags = *rx_payload++;
		...
		{	word32 props_len = 0;
			...
			{	/* Decode the Properties */
				rx_payload += MqttDecode_Props(MQTT_PACKET_TYPE_CONNECT_ACK, &connect_ack->props, rx_payload, props_len);
			}
		}
#endif
	}
...
int MqttEncode_Publish(byte *tx_buf, int tx_buf_len, MqttPublish *publish, byte use_cb)
{	int header_len, variable_len, payload_len = 0;
	...
	if(((publish->buffer != NULL) || (use_cb == 1)) && (publish->total_len > 0)) 
	{	payload_len = publish->total_len;
	}

	/* Encode fixed header */
	publish->type = MQTT_PACKET_TYPE_PUBLISH;
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, variable_len + payload_len, publish->type, publish->retain, publish->qos, publish->duplicate);
	if(header_len < 0) 
	{	return header_len;
	}
	tx_payload = &tx_buf[header_len];
	...
	/* Encode properties */
	tx_payload += MqttEncode_Props(publish->type, publish->props, tx_payload);
#endif
...
{	int header_len, remain_len, variable_len, payload_len;
	...
	/* Decode fixed header */
	publish->type = MQTT_PACKET_TYPE_PUBLISH;
	//printf("%s, rx_buf_len:%d\n", __func__, rx_buf_len);
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, publish->type, &publish->qos, &publish->retain, &publish->duplicate);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	{	word32 props_len = 0;
		...
		if(props_len > 0) 
		{	/* Decode the Properties */
			rx_payload += MqttDecode_Props(publish->type, &publish->props, rx_payload, props_len);
	...
	//printf("%s, payload length:%d, remain_len:%d, variable_len:%d\n", __func__, payload_len, remain_len, variable_len);
...
#ifdef WOLFMQTT_V5
	if((publish_resp->reason_code != MQTT_REASON_SUCCESS) || (publish_resp->props != NULL))
	{	/* Reason Code */
		remain_len++;
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, type, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	{	rx_payload += MqttDecode_Num(rx_payload, &publish_resp->packet_id);
		...
		{	word32 props_len = 0;
			...
			if(props_len > 0) 
			{	/* Decode the Properties */
				rx_payload += MqttDecode_Props((MqttPacketType)type, &publish_resp->props, rx_payload, props_len);
			}
		}
		else 
...
{	int header_len, remain_len, i;
	...
	/* Encode fixed header */
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_SUBSCRIBE, 0, MQTT_QOS_1, 0);
	if(header_len < 0) 
	{	return header_len;
	}
	/* Check for buffer room */
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_SUBSCRIBE_ACK, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	{	rx_payload += MqttDecode_Num(rx_payload, &subscribe_ack->packet_id);
		...
		{	word32 props_len = 0;
			...
			if(props_len > 0) 
			{	/* Decode the Properties */
				rx_payload += MqttDecode_Props(MQTT_PACKET_TYPE_SUBSCRIBE_ACK, &subscribe_ack->props, rx_payload, props_len);
			}
		}
#endif
...
{	int header_len, remain_len, i;
	...
	/* Encode fixed header */
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_UNSUBSCRIBE, 0, MQTT_QOS_1, 0);
	if(header_len < 0) 
	{	return header_len;
	}
	/* Check for buffer room */
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_UNSUBSCRIBE_ACK, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	{	rx_payload += MqttDecode_Num(rx_payload, &unsubscribe_ack->packet_id);
		...
		{	word32 props_len = 0;
			...
			if(props_len > 0)
			{	/* Decode the Properties */
				rx_payload += MqttDecode_Props(MQTT_PACKET_TYPE_UNSUBSCRIBE_ACK, &unsubscribe_ack->props, rx_payload, props_len);
			}
		}
...
{	int header_len, remain_len = 0;
	...
	/* Encode fixed header */
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_PING_REQ, 0, 0, 0);
	if(header_len < 0) 
	{	return header_len;
	}
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_PING_RESP, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
...
{	int header_len;
...
#ifdef WOLFMQTT_V5
	if((disconnect != NULL) && ((disconnect->reason_code != MQTT_REASON_SUCCESS) || (disconnect->props != NULL)))
	...
	/* Encode fixed header */
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_DISCONNECT, 0, 0, 0);
	if(header_len < 0) 
	{	return header_len;
	}
	/* Check for buffer room */
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_DISCONNECT, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	{	/* Decode variable header */
		...
		{	/* Decode Length of Properties */
			...
			{	/* Decode the AUTH Properties */
				rx_payload += MqttDecode_Props(MQTT_PACKET_TYPE_DISCONNECT, &disc->props, rx_payload, props_len);
			}
		}
	}
...
{	int header_len, remain_len = 0;
	...
	/* Encode fixed header */
	header_len = MqttEncode_FixedHeader(tx_buf, tx_buf_len, remain_len, MQTT_PACKET_TYPE_AUTH, 0, 0, 0);
	if(header_len < 0) 
	{	return header_len;
	}
	/* Check for buffer room */
	...
	/* Encode variable header */
	if((auth->reason_code == MQTT_REASON_CONT_AUTH) || (auth->reason_code == MQTT_REASON_REAUTH)) 
...
{	int header_len, remain_len;
	...
	/* Decode fixed header */
	header_len = MqttDecode_FixedHeader(rx_buf, rx_buf_len, &remain_len, MQTT_PACKET_TYPE_AUTH, NULL, NULL, NULL);
	if(header_len < 0) 
	{	return header_len;
	}
	rx_payload = &rx_buf[header_len];
	...
	if((auth->reason_code == MQTT_REASON_SUCCESS) || (auth->reason_code == MQTT_REASON_CONT_AUTH))
	{	/* Decode Length of Properties */
		rx_payload += MqttDecode_Vbi(rx_payload, &props_len);
		if(props_len > 0) 
		{	/* Decode the AUTH Properties */
			rx_payload += MqttDecode_Props(MQTT_PACKET_TYPE_AUTH, &auth->props, rx_payload, props_len);
			if(auth->props != NULL) 
...
{	(void)client;
	...
	//if(rc < 0 && rc != MQTT_CODE_CONTINUE && rc != MQTT_CODE_STDIN_WAKE)
	if((rc < 0) && (rc != MQTT_CODE_CONTINUE) && (rc != MQTT_CODE_WAIT))
	{	/* don't use return code for now - future use */	
		if(rc == MQTT_CODE_WAIT_SOCK_READ_REMAIN_LEN)
		{	if(client->msg_len_err_cb)
			{	client->msg_len_err_cb(client);
			}
			//rc = MQTT_CODE_WAIT;
		}
...
{	int rc;
#ifdef WOLFMQTT_V5
	if((client->packet_sz_max > 0) && (tx_buf_len > (int)client->packet_sz_max))
...
{	int rc, len, remain_read = 0;
	...
	{	case MQTT_PK_BEGIN:
		...
		case MQTT_PK_READ_HEAD:
					{	int i;
						//printf("%s, state = MQTT_PK_READ_HEAD\n", __func__);
						...
						{	/* Check if another byte is needed */
							...
							len = 1;
							//printf("%s 1. sock Read %d\n", __func__, len);
							rc = MqttSocket_Read(client, &rx_buf[client->packet.header_len], len, timeout_ms);
							if(rc < 0) 
							{	return MqttPacket_HandleNetError(client, rc);
							}
							else if(rc != len) 
							{	//printf("%s receive length error rc:%d, len:%d\n", __func__, rc, len);
								return MqttPacket_HandleNetError(client, MQTT_CODE_ERROR_NETWORK);
							}
							client->packet.header_len += len;
						}
						if(i == MQTT_PACKET_MAX_LEN_BYTES) 
						{	return MqttPacket_HandleNetError(client, MQTT_CODE_ERROR_MALFORMED_DATA);
						}
						
						/* Try and decode remaining length */
						rc = MqttDecode_Vbi(header->len, (word32*)&client->packet.remain_len);
		...
		case MQTT_PK_READ:
					{	/* read remainder of packet */
						//printf("%s, MQTT_PK_READ, client->packet.remain_len:%d, client at %p\n", __func__, client->packet.remain_len, client);
            ...
            {	//printf("%s, before MqttSocket_Read(), remain_read:%d, client->packet.header_len:%d\n", __func__, remain_read, client->packet.header_len);
            	rc = MqttSocket_Read(client, &rx_buf[client->packet.header_len], remain_read, timeout_ms);
            	//printf("%s, after MqttSocket_Read(), remain_read:%d, rc:%d\n", __func__, remain_read, rc);
            	if(rc <= 0) 
            	{	return MqttPacket_HandleNetError(client, rc);
            	}
#if 0
            	else if(rc != remain_read)
            	{	client->packet.remain_len -= rc;
            		client->packet.header_len += rc;
            		rc = MQTT_CODE_WAIT_SOCK_READ_REMAIN_LEN;
            		//printf("%s, 1. after MqttSocket_Read(), remain_read:%d, rc:%d, client->packet.remain_len:%d, client->packet.header_len:%d\n", __func__, remain_read, rc, client->packet.remain_len, client->packet.header_len);
            		return MqttPacket_HandleNetError(client, rc);
            	}
#endif
            	remain_read = rc;
