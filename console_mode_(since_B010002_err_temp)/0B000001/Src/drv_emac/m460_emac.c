...
extern void linkDetectThread(void *p);
...
void EMAC_Open(uint8_t *macaddr, struct netif *netif)
{
	...
	sys_thread_new("link_detect_thread", linkDetectThread, netif, THREAD_STACKSIZE, tskIDLE_PRIORITY);
	if(GMACdev.DuplexMode == FULLDUPLEX && GMACdev.Speed == SPEED100)
	{	dbgMsg("Eth Phy Link up, speed: %s\r\n", "100M FULLDUPLEX");
	}
	else if(GMACdev.DuplexMode == HALFDUPLEX && GMACdev.Speed == SPEED100)
	{	dbgMsg("Eth Phy Link up, speed: %s\r\n", "100M HALFDUPLEX");
	}
	else if(GMACdev.DuplexMode == FULLDUPLEX && GMACdev.Speed == SPEED10)
	{	dbgMsg("Eth Phy Link up, speed: %s\r\n", "10M FULLDUPLEX");
	}
	else if(GMACdev.DuplexMode == HALFDUPLEX && GMACdev.Speed == SPEED10)
	{	dbgMsg("Eth Phy Link up, speed: %s\r\n", "10M HALFDUPLEX");
	}
	/*else
	{	dbgMsg("Eth Phy Link up, speed: %s\r\n", "100M FULLDUPLEX");
...
int32_t EMAC_TransmitPkt(uint8_t *pbuf, uint32_t len)
{
    ...
    txdesc = &GMACdev.TxDesc[txnext];
	//if ((pbuf == NULL) || (len == 0) || !synopGMAC_is_desc_empty(txdesc))
	if ((pbuf == NULL) || (len == 0) || synopGMAC_is_desc_owned_by_dma(txdesc))
    {
        return -1;
    }
	safeBuf = &tx_buf[txnext].au8Buf[0];
	memcpy(safeBuf, pbuf, len);
        ...
        offload_needed = 0;
	#endif
    return synopGMAC_xmit_frames(&GMACdev, safeBuf, len, offload_needed, 0);
#else
	if((pbuf != NULL) && (len > 0) &&
...
void linkDetectThread(void *p)
{	struct netif *netif = (struct netif *) p;
	int32_t phy_link_status;
	while(1)
	{	SENS_TIME_DELAY(1000 / portTICK_RATE_MS);
		
		//if(eth_link_status != ETH_LINK_UNDEFINED)
		{	phy_link_status = mii_link_ok(&GMACdev);
			
			if(eth_link_status == ETH_LINK_UP && phy_link_status == LINKDOWN)
				eth_link_status = ETH_LINK_DOWN;
			
			switch(eth_link_status)
			{	case ETH_LINK_UNDEFINED:
						if(phy_link_status == LINKUP)
						{	eth_link_status = ETH_LINK_UP;
							netif_set_link_up(netif);
						}
						break;
